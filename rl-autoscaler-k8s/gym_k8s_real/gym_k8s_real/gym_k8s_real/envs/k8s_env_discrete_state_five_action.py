...
class K8sEnvDiscreteStateFiveAction(discrete.DiscreteEnv):
    metadata = {
        'render.modes': ['human']
    }

    def __init__(self, timestep_duration, app_name, sla_throughput,
    ...
    def step(self, action):
        ...
        if action == 0 and now_observation[1] <= 40:
            return now_observation, 0, self.done, encoded_observation
        elif action == 1 and now_observation[1] <= 20:
            return now_observation, 0, self.done, encoded_observation
        elif action == 3 and now_observation[1] >= 100:
            return now_observation, 0, self.done, encoded_observation
        elif action == 4 and now_observation[1] >= 80:
            return now_observation, 0, self.done, encoded_observation
#         if action == 1:
...
#             return now_observation, reward, self.done, encoded_observation
    ...
    def _take_action(self, action):
        ...
        if action == 0 and pod_cpu_threshold > 40:
            new_cpu_hpa_threshold -= 40
        
        if action == 1 and pod_cpu_threshold > 20:
            new_cpu_hpa_threshold -= 20

        if action == 3 and pod_cpu_threshold < 100:
            new_cpu_hpa_threshold += 20
        
        if action == 4 and pod_cpu_threshold < 80:
            new_cpu_hpa_threshold += 40
