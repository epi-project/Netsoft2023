...
class K8sEnvDQNML(discrete.DiscreteEnv):
    metadata = {
        'render.modes': ['human']
    }
    ...
    def __init__(self, timestep_duration, app_names, app_configs, cluster_names, sla_latency,
            sla_host, sla_latency_metric_name, max_pods, min_pods, app_dict, config_path, changes, connection, pod_weight, latency_weight):
        config.load_kube_config()
        # General variables defining the environment
        # Get following info from k8s
        self.latency_weight = latency_weight
        self.pod_weight = pod_weight
        self.num_cluster = len(cluster_names)
        #connection between different clusters
        ...
        #[firewall], [encrypt], [encrypt]
        self.ip_proxy = [['miss'], ['miss'], ['miss']]
    ...
    def step(self, action_idx):
        ...
        """
        action = self._decode_action(action_idx)
        print("action-idx:{}, config-idx: {}, app_idx: {}, cluster_idx: {}, action: {}, proxy_idx: {}".format(action_idx, action[2], action[4], action[0], action[1], action[3]))
        ...
        if not self._check_proxy():
            wait_time = self.timestep_duration * 10
    ...
    def _get_reward(self, real_observation):
        ...
        reward += self.pod_weight * pod_reward
        ...
        reward += self.latency_weight * throughput_reward
    ...
    def _take_action(self, config_idx, app_idx, cluster_idx, action, proxy_idx):
        ...
        if action == 2:
            ...
            for connection_idx in self.connection[cluster_idx]:
                if self.curr_configs[app_idx][connection_idx][config_idx] == 1:
                    return
        ...
        if action == 0:
            ...
            self._remove_action(config_idx, app_idx, cluster_idx)
    ...
    def _remove_action(self, config_idx, app_idx, cluster_idx):
    ...
    def _reconfig_proxy(self, app_idx, ip, cluster_idx):
        ...
        if len(newIps) > 0:
            for i in range(len(self.ip_proxy)):
                if self.ip_proxy[i][0] == ip:
                    self.ip_proxy[i][0] = random.choice(newIps)
    ...
    def _get_ip(self, app_idx, cluster_idx):
        ...
        for j in range(self.num_configs):
            for conn_idx in self.connection[cluster_idx]:
                if self.curr_configs[app_idx][conn_idx][j] == 1:
                    cluster = self.cluster_names[conn_idx]
                    #the name of deployment
                    deploy_name = self.app_names[app_idx] + sizes[j] 
                    #TO-DO: analyze returned content to get IP
                    serviceIP = ""
                    output = os.popen('kubectl get service -o wide --context=' + cluster).read()
                    lines = output.split("\n")
    ...
    def _create_action(self, config_idx, app_idx, cluster_idx, proxy_idx):
        ...
        for line in lines[:-1]:
            ...
            if items[0][:which_app] == deploy_name:
                serviceIP=items[2]+":"+items[4][:4]
        res = self._assign_proxy(serviceIP, app_idx, proxy_idx)
        return res

    def _assign_proxy(self, ip, app_idx, proxy_idx):
        #assign proxy0 with ip of firewall
        if app_idx == 0 and proxy_idx == 0:
            self.ip_proxy[proxy_idx][0] = ip
    ...
    def _get_state(self):
        ...
        present = []
        for app in self.app_names:
            present.append(self._get_placement(app, self.cluster_names))
